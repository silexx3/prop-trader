...
def ma_pullback(ticker: str, df: pd.DataFrame) -> dict | None:
...
def base_breakout(ticker: str, df: pd.DataFrame, *, base_weeks: int = 4,
                  max_range_pct: float = 10.0, vol_mult: float = 1.5) -> dict | None:
    """≥4 weeks of tight range within ~10% of highs; entry on break of range
    high with volume above 1.5× the 20-day average.
    """
    bars = base_weeks * 5
    ...
    stop_base = range_low * 0.998
    stop_swing = swing_low(df, lookback=10) * 0.998
...
def scan(frames: dict[str, pd.DataFrame], skip_tickers: set[str] = frozenset()) -> list[dict]:
