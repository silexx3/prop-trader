...
**2026-07-09 — the Forward League (outside the experiment).** Four challenger accounts (`league/*.json`, each a simulated \$5,000) trade the same watchlist forward every session under different rulesets: a no-filters control, a Practice-Lab-following frontrunner, an aggressive-sizing account, and a single-position patience account. This experiment's account remains the verdict account and is never written by league code. League results — forward performance on unseen data — are the designated evidence source for promotion decisions; adopting anything into this charter still requires a dated amendment here.
